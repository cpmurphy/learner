...
class GameEditorIntegrationTest < Minitest::Test
  def test_b4_blunder_detection
    ...
    begin
      ...
      assert best_score > 150, "Best move should have score > 150 centipawns (got #{best_score})"
      ...
      assert score_diff > GameEditor::BLUNDER_THRESHOLD, 
             "Score difference (#{score_diff} cp) should exceed blunder threshold (#{GameEditor::BLUNDER_THRESHOLD} cp). Best: #{best_score} cp, Played: #{played_score} cp"
