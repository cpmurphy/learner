...
describe('buildVariationState', () => {
    it('uses PGN variation when user played the expected move', () => {
        ...
        expect(result.currentVariationSANs).toEqual(['e5', 'Rc5', 'Qf4', 'Nd2']);
        expect(result.variationSANsStartIndex).toBe(0);
    ...
    it('slices validation variation and prepares save payload for alternate good move', () => {
        const result = buildVariationState({
            ...
            userMoveSan: 'f5',
            lastMoveData,
            validationData: { variation_sans: ['f5', 'Nf3', 'Nc6'] }
        });

        expect(result.currentVariationSANs).toEqual(['Nf3', 'Nc6']);
        expect(result.variationSANsStartIndex).toBe(1);
    ...
    it('falls back to user move only when no variation is available', () => {
        const result = buildVariationState({
            ...
            goodMoveUci: 'e7e5',
            userMoveSan: 'd5',
            lastMoveData: { ...lastMoveData, variation_sans: [] },
            validationData: {}
        });

        expect(result.currentVariationSANs).toEqual(['d5']);
        expect(result.variationSANsStartIndex).toBe(0);
        expect(result.savePayload).toBeNull();
    });
    ...
    it('uses validation variation when expected move UCI matches but PGN variation is empty', () => {
        const result = buildVariationState({
            ...
            userMoveSan: 'e5',
            lastMoveData: { ...lastMoveData, variation_sans: [] },
            validationData: { variation_sans: ['e5', 'Nc3'] }
        });

        // UCI matches but variation_sans length is 0, so falls through to validation
        expect(result.currentVariationSANs).toEqual(['Nc3']);
        expect(result.variationSANsStartIndex).toBe(1);
        expect(result.savePayload).toEqual({
            move_index: 33,
